...
### 📈 数据统计与分析 (Statistics)
- ✅ **多维度筛选** - 支持1/3/7/15/30天时间范围，可按任务筛选
- ✅ **核心指标** - 总任务数、执行次数、成功率、响应时间等8大指标
- ✅ **趋势分析** - 每日请求次数与错误率双轴趋势图
- ✅ **状态分布** - 任务状态饼图、HTTP响应码柱状图
- ✅ **错误追踪** - 错误触发趋势面积图，识别问题高发时段
- ✅ **执行记录** - 最近执行记录表格，快速查看执行情况
...
### 🎨 主题切换详细说明 ([THEME_GUIDE.md](THEME_GUIDE.md))
...
系统提供完整的数据统计分析功能，帮助您全面了解API任务的运行情况：

1. **进入数据统计页面**
   - 点击导航栏的「📈 数据统计」菜单

2. **筛选数据范围**
   - **时间范围**：选择1天、3天、7天、15天或30天
   - **任务筛选**：可选择特定任务或查看所有任务
   - 点击「刷新数据」按钮手动更新

3. **查看核心指标**
   - 8个彩色统计卡片实时显示关键数据
   - 包括：总任务数、运行中、执行次数、警报总数、成功/失败次数、成功率、平均响应时间

4. **分析数据趋势**
   - **每日趋势图**：查看请求次数和错误率的变化趋势
   - **状态分布图**：了解任务状态占比情况
   - **响应码分布**：分析HTTP状态码分布，识别异常
   - **错误趋势图**：追踪错误发生频率，定位问题时段

5. **查看执行记录**
   - 底部表格显示最近10条执行记录
   - 包含任务名称、URL、状态码、响应时间等详细信息

### 🎨 主题切换使用

系统支持两种视觉主题，可根据个人喜好随时切换：
...
### 数据统计接口
```http
GET    /api/statistics              # 获取完整统计数据（支持days和taskId参数）
GET    /api/statistics/tasks        # 获取任务概览
GET    /api/statistics/executions   # 获取执行概览
GET    /api/statistics/alerts       # 获取警报概览
GET    /api/statistics/status-distribution  # 获取任务状态分布
GET    /api/statistics/daily-trend  # 获取每日趋势数据
GET    /api/statistics/response-codes  # 获取响应码分布
GET    /api/statistics/recent       # 获取最近执行记录
GET    /api/statistics/errors       # 获取错误统计（支持days和taskId参数）
```
